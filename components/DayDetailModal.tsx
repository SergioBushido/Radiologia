...
export default function DayDetailModal({ date, shift, usersMap, userRole, currentUserId, preferences = [], vacations = [], onClose, onEdit, onBlock, onLock, disableLock, onPreferencesUpdated, onEditPreference, onVacationsUpdated }: Props) {
    ...
    async function toggleBlockingDay(userId: number, type: 'VACATION' | 'COURSE') {
        const token = localStorage.getItem('token')
        ...
        if (existing) {
            ...
            if (res.ok) {
                addToast(`${type === 'VACATION' ? 'Vacación' : 'Curso'} eliminado`, 'success')
                onVacationsUpdated?.()
            } else {
                addToast('Error al eliminar', 'error')
            }
            ...
            if (res.ok) {
                addToast(`${type === 'VACATION' ? 'Vacación' : 'Curso'} añadido`, 'success')
                onVacationsUpdated?.()
            } else {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4 transition-all" onClick={onClose}>
            <div className={`bg-[var(--bg-surface)] rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-white/10 flex flex-col max-h-[90vh]`} onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                    ...
                    {(vacations.length > 0 || preferences.some(p => p.type === 'LOCK')) && (
                        <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-white/5">
                            ...
                            <div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                                {vacations.map(vac => (
                                    <div key={vac.id} className={`flex justify-between items-center p-2.5 rounded-xl border text-xs ${vac.type === 'COURSE' ? 'bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/20' : 'bg-green-50 dark:bg-green-500/10 border-green-200 dark:border-green-500/20'}`}>
                                        <div className="flex flex-col">
                                            <span className={`font-bold ${vac.type === 'COURSE' ? 'text-amber-700 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>{usersMap[vac.userId] || `Usuario ${vac.userId}`}</span>
                                            <span className={`text-[10px] ${vac.type === 'COURSE' ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400'}`}>{vac.type === 'COURSE' ? 'Cursos' : 'Vacaciones'}</span>
                                        </div>
                                ...
                                {preferences.filter(p => p.type === 'LOCK').map(pref => (
                                    <div key={pref.id} className="flex justify-between items-center p-2.5 bg-red-50 dark:bg-red-500/10 rounded-xl border border-red-200 dark:border-red-500/20 text-xs">
                                        ...
                                        {(userRole === 'ADMIN' || pref.userId === currentUserId) && (
                    ...
                    {userRole === 'ADMIN' && preferences.some(p => p.type !== 'LOCK') && (
                        <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-100 dark:border-white/5">
                        <button
                            ...
                            className="flex flex-col items-center justify-center gap-1 p-3 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-xl font-bold hover:bg-green-200 dark:hover:bg-green-900/50 transition-all active:scale-95 border border-green-200 dark:border-green-500/20"
                        >
                        ...
                        <button
                            ...
                            className="flex flex-col items-center justify-center gap-1 p-3 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-xl font-bold hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-all active:scale-95 border border-amber-200 dark:border-amber-500/20"
                        >
                        ...
                        <button
                            onClick={toggleLock}
                            className={`flex flex-col items-center justify-center gap-1 p-3 rounded-xl font-bold transition-all border text-[10px] ${myLock
